...
! Wave estimate
  real(8) :: lg,roh,cs2,bb,sig,hs,ca2,om2,rr,vsq
  real(8) :: l1,l2,l3,lf,ls,cfx,cfy,cfz
...
! calculate time step dt in resistive glmmhd @ cartesian grid
  ...
  do k=1,kx
     do j=1,jx
        do i=1,ix
          lg = dsqrt(1d0+vx(i,j,k)*vx(i,j,k)+vy(i,j,k)*vy(i,j,k)+vz(i,j,k)*vz(i,j,k))
          roh = ro(i,j,k) + pr(i,j,k)*gm/(gm-1d0)
          cs2 = gm*pr(i,j,k)/roh

          bb  = (bx(i,j,k)*bx(i,j,k)+by(i,j,k)*by(i,j,k)+bz(i,j,k)*bz(i,j,k))/(lg*lg) &
              + ((vx(i,j,k)*bx(i,j,k)+by(i,j,k)*vy(i,j,k)+bz(i,j,k)*vz(i,j,k))/lg)**2
          sig = bb/ro(i,j,k)
          hs  = roh/ro(i,j,k) + sig
          ...
          rr  = cs2*(((vx(i,j,k)*bx(i,j,k)+by(i,j,k)*vy(i,j,k)+bz(i,j,k)*vz(i,j,k))/lg)**2)/(ro(i,j,k)*hs*lg*lg)

          vsq = (vx(i,j,k)*vx(i,j,k)+vy(i,j,k)*vy(i,j,k)+vz(i,j,k)*vz(i,j,k))/(lg*lg)

          !! X-direction Wave Speed
          l1 = vx(i,j,k)*(1d0-om2)/lg
          l2 = (1d0-om2*vsq-rr)
          l3 = ((vsq-1d0)*om2+rr)*( (vsq-vx(i,j,k)*vx(i,j,k)/(lg*lg))*om2 + (vx(i,j,k)*vx(i,j,k))/(lg*lg) -1d0 + rr )
          lf = l1/l2 + dsqrt(l3)/l2
          ls = l1/l2 - dsqrt(l3)/l2
          cfx = max(dabs(lf),dabs(ls))

          !! Y-direction Wave Speed
          l1 = vy(i,j,k)*(1d0-om2)/lg
          l2 = (1d0-om2*vsq-rr)
          l3 = ((vsq-1d0)*om2+rr)*( (vsq-vy(i,j,k)*vy(i,j,k)/(lg*lg))*om2 + (vy(i,j,k)*vy(i,j,k))/(lg*lg) -1d0 + rr )
          lf = l1/l2 + dsqrt(l3)/l2
          ls = l1/l2 - dsqrt(l3)/l2
          cfy = max(dabs(lf),dabs(ls))

          !! Z-direction Wave Speed
          l1 = vz(i,j,k)*(1d0-om2)/lg
          l2 = (1d0-om2*vsq-rr)
          l3 = ((vsq-1d0)*om2+rr)*( (vsq-vz(i,j,k)*vz(i,j,k)/(lg*lg))*om2 + (vz(i,j,k)*vz(i,j,k))/(lg*lg) -1d0 + rr )
          lf = l1/l2 + dsqrt(l3)/l2
          ls = l1/l2 - dsqrt(l3)/l2
          cfz = max(dabs(lf),dabs(ls))
