module lr_state
  ...
  public :: lr_state__1st, lr_state__MSCL2, lr_state__MP5 &
  ,reconstructionConstant, reconstructionConstant_cyl
...
contains
  ...
  integer :: i,j,k,l,n
  real(8),dimension(nwave) :: qql,qqr,qqlm,qqrm ! qqlm,qqrm for minmod
...
!----Shock Flatting  ---
  real(8), parameter :: eps=5d0
  real(8) :: vsh1,vsh2,rr
  real(8), dimension(3) :: kaix,kaiy,kaiz
  ...
  if(mdir == 1)then
     ...
     !$OMP         qql,qqr,qqlm,qqrm,minvalue,romaxvalue,prmaxvalue,smv,psmv,msmv,&
     !$OMP         vsh1,vsh2,kaix,kaiy,kaiz)
     do k=margin,kx-margin+1
        do j=margin,jx-margin+1
           do i=margin,ix-margin+1
              ...
              do n=1,nwave
                ...
                dql = (ww(n,2)-ww(n,1))/dxl
                dqr = (ww(n,3)-ww(n,2))/dxr
                dqc = (ww(n,3)-ww(n,1))/dxc

                ! *** MC limiter *** !
                ! dqqx = MC2(2.*dqr,2.*dql,dqc)
                ! ! right-hand left-state
                ! qqr(n) = ww(n,2) + 0.5d0*dqqx*dx(i)
                ! !left-hand right-state
                ! qql(n) = ww(n,2) - 0.5d0*dqqx*dx(i)
                
                ! ****Van Leer*****!
                dqqx = (2d0*max(dql*dqr,0d0))/(dql+dqr)
                if( isnan(dqqx)  )then
                   dqqx = 0d0
                endif                
                qqr(n) = ww(n,2) + 0.5d0*dqqx*dx(i)
                ...
                dqqx = minmod(dql,dqr)
                !right-hand left-state
                qqrm(n) = ww(n,2) + 0.5d0*dqqx*dx(i)
                !left-hand right-state
                qqlm(n) = ww(n,2) - 0.5d0*dqqx*dx(i)
              ...
              !******* Shock flattening (Mignone and Bodo 2005) *******!
              vsh1 = (vx(i+1,j,k) - vx(i-1,j,k))/(2d0*dx(i)) &
                   + (vy(i,j+1,k) - vy(i,j-1,k))/(2d0*dy(j)) &
                   + (vz(i,j,k+1) - vz(i,j,k-1))/(2d0*dz(k))
              vsh1 = sign(1d0,vsh1) ! shock finding vsh1 = -1d0
              vsh1 = max(0d0,-vsh1) ! shock yes -> vsh1=1d0, No.-> vsh1= 0d0 

              kaix(1) = (dabs(pr(i,j,k)   - pr(i-2,j,k)))/min(pr(i,j,k)  ,pr(i-2,j,k))
              kaix(2) = (dabs(pr(i+1,j,k) - pr(i-1,j,k)))/min(pr(i+1,j,k),pr(i-1,j,k))
              kaix(3) = (dabs(pr(i+2,j,k) - pr(i  ,j,k)))/min(pr(i+2,j,k),pr(i  ,j,k))
              kaix(1) = sign(1d0,eps-kaix(1))
              kaix(2) = sign(1d0,eps-kaix(2))
              kaix(3) = sign(1d0,eps-kaix(3))
              kaix(1) = max(0d0,kaix(1))
              kaix(2) = max(0d0,kaix(2))
              kaix(3) = max(0d0,kaix(3))

              kaiy(1) = (dabs(pr(i,j,k)   - pr(i,j-2,k)))/min(pr(i,j,k)  ,pr(i,j-2,k))
              kaiy(2) = (dabs(pr(i,j+1,k) - pr(i,j-1,k)))/min(pr(i,j+1,k),pr(i,j-1,k))
              kaiy(3) = (dabs(pr(i,j+2,k) - pr(i  ,j,k)))/min(pr(i,j+2,k),pr(i  ,j,k))
              kaiy(1) = sign(1d0,eps-kaiy(1))
              kaiy(2) = sign(1d0,eps-kaiy(2))
              kaiy(3) = sign(1d0,eps-kaiy(3))
              kaiy(1) = max(0d0,kaiy(1))
              kaiy(2) = max(0d0,kaiy(2))
              kaiy(3) = max(0d0,kaiy(3))

              kaiz(1) = (dabs(pr(i,j,k)   - pr(i,j,k-2)))/min(pr(i,j,k)  ,pr(i,j,k-2))
              kaiz(2) = (dabs(pr(i,j,k+1) - pr(i,j,k-1)))/min(pr(i,j,k+1),pr(i,j,k-1))
              kaiz(3) = (dabs(pr(i,j,k+2) - pr(i  ,j,k)))/min(pr(i,j,k+2),pr(i  ,j,k))
              kaiz(1) = sign(1d0,eps-kaiz(1))
              kaiz(2) = sign(1d0,eps-kaiz(2))
              kaiz(3) = sign(1d0,eps-kaiz(3))
              kaiz(1) = max(0d0,kaiz(1))
              kaiz(2) = max(0d0,kaiz(2))
              kaiz(3) = max(0d0,kaiz(3))              
              
              vsh2   = min(kaix(1),kaix(2),kaix(3), &
                   kaiy(1),kaiy(2),kaiy(3), &
                   kaiz(1),kaiz(2),kaiz(3)) ! strong shock yes -> vsh2=0d0, No -> 1d0
              msmv   = int(vsh1)*(1-int(vsh2))
              psmv   = 1 - msmv
              ...
              row(i-1,j,k,2) = qql(1)*psmv + qqlm(1)*msmv
              row(i  ,j,k,1) = qqr(1)*psmv + qqrm(1)*msmv
              vxw(i-1,j,k,2) = qql(2)*psmv + qqlm(2)*msmv
              vxw(i  ,j,k,1) = qqr(2)*psmv + qqrm(2)*msmv
              vyw(i-1,j,k,2) = qql(3)*psmv + qqlm(3)*msmv
              vyw(i  ,j,k,1) = qqr(3)*psmv + qqrm(3)*msmv
              vzw(i-1,j,k,2) = qql(4)*psmv + qqlm(4)*msmv
              vzw(i  ,j,k,1) = qqr(4)*psmv + qqrm(4)*msmv
              prw(i-1,j,k,2) = qql(5)*psmv + qqlm(5)*msmv
              prw(i  ,j,k,1) = qqr(5)*psmv + qqrm(5)*msmv
              bxw(i-1,j,k,2) = qql(6)*psmv + qqlm(6)*msmv
              bxw(i  ,j,k,1) = qqr(6)*psmv + qqrm(6)*msmv
              byw(i-1,j,k,2) = qql(7)*psmv + qqlm(7)*msmv
              byw(i  ,j,k,1) = qqr(7)*psmv + qqrm(7)*msmv
              bzw(i-1,j,k,2) = qql(8)*psmv + qqlm(8)*msmv
              bzw(i  ,j,k,1) = qqr(8)*psmv + qqrm(8)*msmv
              phiw(i-1,j,k,2) = qql(9)*psmv + qqlm(9)*msmv
              phiw(i  ,j,k,1) = qqr(9)*psmv + qqrm(9)*msmv
           end do
        end do
     end do
!$OMP END PARALLEL DO
        ...
        do j=margin,jx-margin+1
           do i=margin,ix-margin+1
              ...
              do n=1,nwave
                 ...
                 dql = (ww(n,2)-ww(n,1))/dyl
                 dqr = (ww(n,3)-ww(n,2))/dyr
                 dqc = (ww(n,3)-ww(n,1))/dyc

                !*** MC limitel ***!
                 ! dqqy = MC2(2.*dqr,2.*dql,dqc)
                 ! !right-hand left-state
                 ! qqr(n) = ww(n,2) + 0.5d0*dqqy*dy(j)
                 ! !left-hand right-state
                 ! qql(n) = ww(n,2) - 0.5d0*dqqy*dy(j)

                ! ****Van Leer*****!
                dqqy = (2d0*max(dql*dqr,0d0))/(dql+dqr)
                if( isnan(dqqy)  )then
                   dqqy = 0d0
                endif                
                qqr(n) = ww(n,2) + 0.5d0*dqqy*dy(j)
                qql(n) = ww(n,2) - 0.5d0*dqqy*dy(j)
                 ...
                 !right-hand left-state
                 qqrm(n) = ww(n,2) + 0.5d0*dqqy*dy(j)
                 !left-hand right-state
                 qqlm(n) = ww(n,2) - 0.5d0*dqqy*dy(j)
              ...
              row(i,j-1,k,2) = qql(1)*psmv + qqlm(1)*msmv
              row(i,j  ,k,1) = qqr(1)*psmv + qqrm(1)*msmv
              vxw(i,j-1,k,2) = qql(2)*psmv + qqlm(2)*msmv
              vxw(i,j  ,k,1) = qqr(2)*psmv + qqrm(2)*msmv
              vyw(i,j-1,k,2) = qql(3)*psmv + qqlm(3)*msmv
              vyw(i,j  ,k,1) = qqr(3)*psmv + qqrm(3)*msmv
              vzw(i,j-1,k,2) = qql(4)*psmv + qqlm(4)*msmv
              vzw(i,j  ,k,1) = qqr(4)*psmv + qqrm(4)*msmv
              prw(i,j-1,k,2) = qql(5)*psmv + qqlm(5)*msmv
              prw(i,j  ,k,1) = qqr(5)*psmv + qqrm(5)*msmv
              bxw(i,j-1,k,2) = qql(6)*psmv + qqlm(6)*msmv
              bxw(i,j  ,k,1) = qqr(6)*psmv + qqrm(6)*msmv
              byw(i,j-1,k,2) = qql(7)*psmv + qqlm(7)*msmv
              byw(i,j  ,k,1) = qqr(7)*psmv + qqrm(7)*msmv
              bzw(i,j-1,k,2) = qql(8)*psmv + qqlm(8)*msmv
              bzw(i,j  ,k,1) = qqr(8)*psmv + qqrm(8)*msmv
              phiw(i,j-1,k,2) = qql(9)*psmv + qqlm(9)*msmv
              phiw(i,j  ,k,1) = qqr(9)*psmv + qqrm(9)*msmv
           end do
        end do
     end do
        ...
        do j=margin,jx-margin+1
           do i=margin,ix-margin+1
              ...
              do n=1,nwave
                 ...
                 dzc = dzl+dzr
                 dql = (ww(n,2)-ww(n,1))/dzl
                 dqr = (ww(n,3)-ww(n,2))/dzr
                 dqc = (ww(n,3)-ww(n,1))/dzc
                 
                 !*** MC limiter ***!
                 ! dqqz = MC2(2.*dqr,2.*dql,dqc)
                 ! !right-hand left-state
                 ! qqr(n) = ww(n,2) + 0.5d0*dqqz*dz(k)
                 ! !left-hand right-state
                 ! qql(n) = ww(n,2) - 0.5d0*dqqz*dz(k)
                 
                 ! ***Van Leer*****!
                 dqqz = (2d0*max(dql*dqr,0d0))/(dql+dqr)
                 if( isnan(dqqz))then
                    dqqz=0d0
                 endif
                 qqr(n) = ww(n,2) + 0.5d0*dqqz*dz(k)
                 ...
                 !*** minmod limiter  ***!
                 dqqz = minmod(dqr,dql)
                 qqrm(n) = ww(n,2) + 0.5d0*dqqz*dz(k)
                 qqlm(n) = ww(n,2) - 0.5d0*dqqz*dz(k)
              end do
              !******* Shock flattening (Mignone and Bodo 2005) *******!
              vsh1 = (vx(i+1,j,k) - vx(i-1,j,k))/(2d0*dx(i)) &
                   + (vy(i,j+1,k) - vy(i,j-1,k))/(2d0*dy(j)) &
              ...
              row(i,j,k-1,2) = qql(1)*psmv  + qqlm(1)*msmv
              row(i,j,k  ,1) = qqr(1)*psmv  + qqrm(1)*msmv
              vxw(i,j,k-1,2) = qql(2)*psmv  + qqlm(2)*msmv
              vxw(i,j,k  ,1) = qqr(2)*psmv  + qqrm(2)*msmv
              vyw(i,j,k-1,2) = qql(3)*psmv  + qqlm(3)*msmv
              vyw(i,j,k  ,1) = qqr(3)*psmv  + qqrm(3)*msmv
              vzw(i,j,k-1,2) = qql(4)*psmv  + qqlm(4)*msmv
              vzw(i,j,k  ,1) = qqr(4)*psmv  + qqrm(4)*msmv
              prw(i,j,k-1,2) = qql(5)*psmv  + qqlm(5)*msmv
              prw(i,j,k  ,1) = qqr(5)*psmv  + qqrm(5)*msmv
              bxw(i,j,k-1,2) = qql(6)*psmv  + qqlm(6)*msmv
              bxw(i,j,k  ,1) = qqr(6)*psmv  + qqrm(6)*msmv
              byw(i,j,k-1,2) = qql(7)*psmv  + qqlm(7)*msmv
              byw(i,j,k  ,1) = qqr(7)*psmv  + qqrm(7)*msmv
              bzw(i,j,k-1,2) = qql(8)*psmv  + qqlm(8)*msmv
              bzw(i,j,k  ,1) = qqr(8)*psmv  + qqrm(8)*msmv
              phiw(i,j,k-1,2) = qql(9)*psmv + qqlm(9)*msmv
              phiw(i,j,k  ,1) = qqr(9)*psmv + qqrm(9)*msmv
           end do
        end do
     end do
  ...
  integer,parameter :: nwave = 9
  real(8),dimension(nwave) :: qql,qqr,qqlm,qqrm ! qqlm,qqrm for minmod limiter at storng shock
  real(8),dimension(nwave,2) :: wwc_w
  ! ww(*,1) = qqm2, ..
     ...
     do k=3,kx-2
        do j=3,jx-2
           ...
           do i=3,ix-2
              ...
              do n=1,nwave
                 !right-hand left state
                 wwor = (ccx(1,2,i)*ww(n,1)+ccx(2,2,i)*ww(n,2) &
                      + ccx(3,2,i)*ww(n,3) + ccx(4,2,i)*ww(n,4) &
                      + ccx(5,2,i)*ww(n,5))

                 djm1 = ww(n,1)-2.0d0*ww(n,2)+ww(n,3)
                 dj = ww(n,2)-2.0d0*ww(n,3)+ww(n,4)
                 djp1 = ww(n,3)-2.0d0*ww(n,4)+ww(n,5)

                 dm4jph = minmod4(4.0d0*dj-djp1,4.0d0*djp1-dj,dj,djp1)
                 dm4jmh = minmod4(4.0d0*dj-djm1,4.0d0*djm1-dj,dj,djm1)

                 qqul = ww(n,3)+Alpha*(ww(n,3)-ww(n,2))
                 qqmd = 0.5d0*(ww(n,3)+ww(n,4) - dm4jph)
                 qqlc = ww(n,3) + 0.5d0*(ww(n,3)-ww(n,2)) + B2*dm4jmh

                 qqmin = max(min(ww(n,3),ww(n,4),qqmd),min(ww(n,3),qqul,qqlc))
                 qqmax = min(max(ww(n,3),ww(n,4),qqmd),max(ww(n,3),qqul,qqlc))
                 qqr(n) = wwor + minmod((qqmin-wwor),(qqmax-wwor))

                 !left-hand right state
                 wwor = (ccx(5,1,i)*ww(n,5)+ccx(4,1,i)*ww(n,4) &
                      + ccx(3,1,i)*ww(n,3) + ccx(2,1,i)*ww(n,2) &
                      + ccx(1,1,i)*ww(n,1))

                 qqlr = ww(n,3)+Alpha*(ww(n,3)-ww(n,4))
                 qqmd = 0.5d0*(ww(n,3)+ww(n,2) - dm4jmh)
                 qqlc = ww(n,3) + 0.5d0*(ww(n,3)-ww(n,4)) + B2*dm4jph

                 qqmin = max(min(ww(n,3),ww(n,2),qqmd),min(ww(n,3),qqlr,qqlc))
                 qqmax = min(max(ww(n,3),ww(n,2),qqmd),max(ww(n,3),qqlr,qqlc))
                 qql(n) = wwor + minmod((qqmin-wwor),(qqmax-wwor))

                 ! **** minmod swiching at strong shock *** !
                 dql = (ww(n,3)-ww(n,2))/(0.5d0*(dx(i)+dx(i-1)))
                 dqr = (ww(n,4)-ww(n,3))/(0.5d0*(dx(i+1)+dx(i)))                 
                 dqqx = minmod(dql,dqr)
                 qqrm(n) = ww(n,3) + 0.5d0*dqqx*dx(i)
                 qqlm(n) = ww(n,3) - 0.5d0*dqqx*dx(i)                 
              end do
              ...
              vsh2   = min(kaix(1),kaix(2),kaix(3),kaiy(1),kaiy(2),kaiy(3)) ! strong shock yes -> vsh2=0d0, No -> 1d0
              msmv   = int(vsh1)*(1-int(vsh2))
              psmv   = 1 - msmv
     ...
     do k=3,kx-2
        do i=3,ix-2
           ...
           do j=3,jx-2
              ...
              do n=1,nwave
                 ! left state
                 wwor = (ccy(1,2,j)*ww(n,1)+ccy(2,2,j)*ww(n,2) &
                      + ccy(3,2,j)*ww(n,3) + ccy(4,2,j)*ww(n,4) &
                      + ccy(5,2,j)*ww(n,5))
                 ...
                 ! right state
                 wwor = (ccy(5,1,j)*ww(n,5)+ccy(4,1,j)*ww(n,4) &
                      + ccy(3,1,j)*ww(n,3) + ccy(2,1,j)*ww(n,2) &
                      + ccy(1,1,j)*ww(n,1))
     ...
     do j=3,jx-2
        do i=3,ix-2
           ...
           do k=3,kx-2
              ...
              do n=1,nwave
                 ! left state
                 wwor = (ccz(1,2,k)*ww(n,1)+ccz(2,2,k)*ww(n,2) &
                      + ccz(3,2,k)*ww(n,3) + ccz(4,2,k)*ww(n,4) &
                      + ccz(5,2,k)*ww(n,5))
              ...
              ! right state
                 wwor = (ccz(5,1,k)*ww(n,5)+ccz(4,1,k)*ww(n,4) &
                      + ccz(3,1,k)*ww(n,3) + ccz(2,1,k)*ww(n,2) &
                      + ccz(1,1,k)*ww(n,1))
